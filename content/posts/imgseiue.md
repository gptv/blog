...
R2_ENDPOINT="https://<Urname>.r2.cloudflarestorage.com"
...
SRC_DIR="/Users/suen/Pictures/blog-images"
...
check_dependencies() {
  ...
  for cmd in rclone cwebp; do
    if ! command -v "$cmd" &> /dev/null; then
      echo "錯誤：找不到必要的命令 '$cmd'。" >&2
      missing=1
    fi
  done
  # viu 是可選的，用於預覽
  if ! command -v viu &> /dev/null; then
      echo "提示：未找到 'viu' 命令。圖片預覽功能將不可用。" >&2
      echo "      可以通過 'brew install viu' (如果使用 Homebrew) 來安裝它。" >&2
  fi
...
DEST_RCLONE_PATH="${RCLONE_REMOTE_NAME}:${R2_BUCKET_NAME}/" # 指向 Bucket 根目錄
OUT_DIR=$(mktemp -d -t converted_images_XXXXXX)
trap 'log "清理臨時目錄 $OUT_DIR..."; rm -rf "$OUT_DIR"' EXIT
log "使用臨時目錄：$OUT_DIR"
log "源圖片目錄：$SRC_DIR"
log "已處理圖片將移至：$PROCESSED_DIR"
log "圖片將上傳到 R2 Bucket 根目錄：$DEST_RCLONE_PATH"
log "Markdown 文件：$MD_FILE"

# 處理 Markdown 文件
MD_NEEDS_UPDATE=0
if [[ ! -f "$MD_FILE" ]]; then
  log "創建 Markdown 文件：$MD_FILE"
  echo -e "\n### $DATE_SUFFIX\n" > "$MD_FILE"; MD_NEEDS_UPDATE=1
else
  if ! grep -q "^### $DATE_SUFFIX" "$MD_FILE"; then
      log "向 Markdown 文件 $MD_FILE 添加日期標題..."
      echo -e "\n### $DATE_SUFFIX\n" >> "$MD_FILE"; MD_NEEDS_UPDATE=1
  fi
fi

# 獲取當天已存在的最大序號並設置起始 COUNT
...
for IMG in "${IMAGE_FILES[@]}"; do
  ...
  if ! cwebp -q "$IMG_QUALITY" "$IMG" -o "$NEW_IMG_WEBP"; then
    log "警告：轉換圖片 '$ORIG_FILENAME' 失敗，跳過此文件。"
    continue
  fi
  ...
  if ! rclone --config "$RCLONE_CONFIG_FILE" copy "$NEW_IMG_WEBP" "$DEST_RCLONE_PATH" --progress; then
     log "警告：上傳圖片 '$BASE_NAME.webp' 失敗。"
     continue
  fi

  # --- 成功處理後的操作 ---
  PUBLIC_URL="${PUBLIC_IMG_BASE_URL}/${BASE_NAME}.webp"
  # *** 修改 Markdown Alt 文本，包含原始文件名 ***
  ...
  if ! mv "$IMG" "$PROCESSED_DIR/"; then
      log "警告：移動原圖 '$ORIG_FILENAME' 失敗，但圖片已上傳。"
  fi

  ((COUNT++)); ((UPLOADED_COUNT++)); MD_NEEDS_UPDATE=1
done

# 將本次運行的所有鏈接追加到 Markdown 文件
...
# --- 在所有日誌之後，輸出本次成功上傳的所有 Markdown 鏈接和預覽 (如果 viu 可用) ---
if [[ "$UPLOADED_COUNT" -gt 0 ]]; then
    echo # 輸出一個空行分隔
    echo "--- 本次成功上傳的鏈接與預覽 ---"
    # 檢查 viu 是否存在
    if command -v viu &> /dev/null; then
        ...
        for (( i=0; i<${#MD_LINKS_ARRAY[@]}; i++ )); do
            local link="${MD_LINKS_ARRAY[i]}"
            local preview_path="${PREVIEW_FILES_ARRAY[i]}"
            echo # 每個條目前加空行
            printf "%s\n" "$link" # 打印 Markdown 鏈接
            # 調用 viu 顯示預覽，限制寬度
